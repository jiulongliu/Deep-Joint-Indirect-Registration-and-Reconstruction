...
#        moving_image_var=Variable(torch.from_numpy(moving_image_np).view(-1,1,192,192).cuda())
        target_image_var=Variable(torch.from_numpy(target_image_np).view(-1,1,192,192).cuda())        
#        noiselevel = 0.1
        ...
        Y_part = M.gensindata(moving_image_np.reshape((-1,args.image_size_nx,args.image_size_ny)), noiselevel = args.noise_level)  
        ...
        Y_batch_var = Variable(Y_part_t).cuda()       #.view(-1,args.image_size_nx,args.image_size_ny) 
...
#            m0_reg = common.FieldFromNPArr(m0, mType);
        ...
        M.__init__(nx=args.image_size_nx,ny=args.image_size_ny,nd=args.sinogram_size_nd,nv=args.sinogram_size_nv,SO=1000,OD=500,offset=0, mu=0.8,rho=0.4,lam=0.08,N_iter=20,CG_iter=20,CG_tol=1e-12,Min_iter=10)

#        mv_rec_w_mvp3 = M.recon_admm_batch(Y_part, mvimgp3)
#        mv_rec_w_m0p3 = M.recon_admm_batch(Y_part, I1_rec)
        scipy.io.savemat(args.output_prefix[i]+'.mat', {'m0_p3': m0,'m0_p1': m01, 'm0_p2': m02,'m0p1t': m01t,  

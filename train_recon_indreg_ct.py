...
parser.add_argument('--image-size-ny', type=int, default=192, metavar='N',
                    help='image width')
parser.add_argument('--sinogram-size-nd', type=int, default=512, metavar='N',
                    help='number of detectors')
parser.add_argument('--sinogram-size-nv', type=int, default=18, metavar='N',
                    help='number of views')
parser.add_argument('--noise-level', type=float, default=0.0, metavar='N',
                    help='noise level')
...
def create_net(args):
    M=[]
    for i in range(0, 3):
        Mi=pyctrecon.ctoplib(nx=args.image_size_nx,ny=args.image_size_ny,nd=args.sinogram_size_nd,nv=args.sinogram_size_nv,SO=1000,OD=500,offset=0,mu=0.0,rho=0.4,lam=0.08,N_iter=0,CG_iter=40,CG_tol=1e-12,Min_iter=10)
        M.append(Mi)    
...
#    M = net.invblk_1.M
    M=pyctrecon.ctoplib(nx=args.image_size_nx,ny=args.image_size_ny,nd=args.sinogram_size_nd,nv=args.sinogram_size_nv,SO=1000,OD=500,offset=0, mu=1.0,rho=0.0,lam=0.08,N_iter=20,CG_iter=20,CG_tol=1e-12,Min_iter=10)
    Y_part = M.gensindata(moving_appear_trainset.view(-1,args.image_size_nx,args.image_size_ny).numpy(), noiselevel = noiselevel)  
...
#                                                 'Y_part':Y_part,
#                                                 'moving_appear_trainset':moving_appear_trainset.view(-1,args.image_size_nx,args.image_size_ny).numpy()})    
    ...
    for i, (mv_batch,tg_batch,output_batch,Y_batch,mvrc0_batch) in enumerate(train_loader):
        ...
        print('====> Epoch: {}, datapart: {}, iter: {}/{}, recon+reg loss: {:.10f}, rho1: {}, rho2: {}, rho3: {}'.format(
        ...
        if i % 100 == 0 or i == N-1:
            ...
            if (cur_epoch+1)%50==0:
                torch.save(model_info, "output/train/prediction_ct_recon_indreg_"+str(args.sinogram_size_nv)+"_ep_"+str(cur_epoch+1)+".pth.tar")
                scipy.io.savemat("output/train/imtrain_training_ct_"+str(args.sinogram_size_nv)+"_ep_"+str(cur_epoch+1)+".mat", {'m0p1': m0p1_batch_variable.detach().view(-1,2,args.image_size_nx, args.image_size_ny).cpu().numpy(),
                                                 ...
                                                 'tggd': tg_batch_variable.detach().view(-1,1,args.image_size_nx, args.image_size_ny).cpu().numpy(),
                                                 'rho1': net.invblk_1.M.rho,'rho2': net.invblk_2.M.rho,'rho3': net.invblk_3.M.rho})#enddef    
